---
datasheet: https://thinksrs.com/downloads/pdfs/catalog/CS580c.pdf
group: Voltage & Current Sources
image: https://thinksrs.com/images/instr/cs580/CS580%20Shadow.jpg
is_new: false
manual: https://thinksrs.com/downloads/pdfs/manuals/CS580m.pdf
model: CS580
name: Current Source
slug: cs580
sort: 10
tab: sci
tile_desc: Ultra-low-noise precision current source for resistance measurements and
  sensitive experiments.

troubleshooting:
  - q: "Instrument is unresponsive to front-panel button presses"
    a: "Try a power-up reset. Turn the power off, then while holding the INPUT key down, turn the power back on. After two seconds release the INPUT key. If this doesn't correct the problem the instrument will need to come back to SRS for service. Click here to get an RMA number."

description_bullets:
- AC and DC current source
- Sources/sinks current ( true four quadrant operation )
- Current from 100 fA to 100 mA
- Up to 200 kHz bandwidth
- ±50 V compliance voltage
- 1 nA/V to 50 mA/V gain
- Low thermal drift
- RS-232 & Optical fiber interfaces
price: $4495
price_note: FOB Sunnyvale, CA
quick_specs:
- label: Compliance voltage
  value: 0 to 50 V (bipolar)
- label: Compliance voltage error
  value: 0.5 % + 50 mV
- label: DC output resistance
  value: 10 12 Ω (1 nA/V gain)
- label: Output capacitance
  value: <12 pF (filter off), <500 pF (filter on)
- label: Guard output
  value: -50 V to +50 V, 5 kΩ internal resistance
- label: Output power
  value: 5 W (four quadrant sourcing/sinking)
spec_groups:
- name: Current Output
  rows:
  - - Compliance voltage
    - 0 to 50 V (bipolar)
  - - Compliance voltage error
    - 0.5 % + 50 mV
  - - DC output resistance
    - 10 12 Ω (1 nA/V gain)
  - - Output capacitance
    - <12 pF (filter off), <500 pF (filter on)
  - - Guard output
    - -50 V to +50 V, 5 kΩ internal resistance
  - - Output power
    - 5 W (four quadrant sourcing/sinking)
  - - THD
    - 0.01 % typ.
  - - Output connector
    - 3-lug Triax for current output. Banana jacks for load voltage monitoring
  - - Common mode voltage
    - 250 Vrms (DC to 60 Hz)
  - - Common mode isolation
    - '>1 GΩ, <1 nF'
- name: Input
  rows:
  - - Input range
    - -2 V to +2 V
  - - Input impedance
    - 100 kΩ
  - - Input connector
    - BNC
- name: Monitor Output
  rows:
  - - Source impedance
    - 5 kΩ
  - - Output bandwidth
    - 20 kHz (typ., into high impedance)
- name: Remote Interfaces
  rows:
  - - RS-232
    - DB-9 connector, 9600 baud
  - - Optical fiber
    - Connection to SX199 Optical Interface Controller . Provides connectivity to
      GPIB, RS-232 and Ethernet
- name: General
  rows:
  - - Operating temperature
    - 0 °C to 40 °C, non-condensing
  - - Power
    - <30 W, 100/120/220/240 VAC, 50 Hz or 60 Hz
  - - Dimensions
    - 8.3 × 3.5 × 13.0 (WHL)
  - - Weight
    - 10 lbs.
  - - Warranty
    - One year parts and labor on defects in materials and workmanship
- name: BNC analog input (AC & DC)
  rows:
  - - Gain
    - Max Output
  - - 1 nA/V
    - 2.2 nA
  - - 10 nA/V
    - 22 nA
  - - 100 nA/V
    - 220 nA
  - - 1 μA/V
    - 2.2 μA
  - - 10 μA/V
    - 22 μA
  - - 100 μA/V
    - 220 μA
  - - 1 mA/V
    - 2.2 mA
  - - 10 mA/V
    - 22 mA
  - - 50 mA/V
    - 110 mA
  - - Gain drift
    - 200 ppm/°C
  - - Offset
    - 3 mV (referred to input)
  - - Offset drift
    - 1 mV/°C (referred to input)
- name: Internal DC current source (analog input disabled)
  rows:
  - - Gain
    - Max Output
  - - 1 nA/V
    - 2 nA
  - - 10 nA/V
    - 20 nA
  - - 100 nA/V
    - 200 nA
  - - 1 μA/V
    - 2 μA
  - - 10 μA/V
    - 20 μA
  - - 100 μA/V
    - 200 μA
  - - 1 mA/V
    - 2 mA
  - - 10 mA/V
    - 20 mA
  - - 50 mA/V
    - 100 mA
- name: Noise
  rows:
  - - Gain
    - Noise density (Input off)
  - - 1 nA/V
    - 10 fA/√Hz
  - - 10 nA/V
    - 20 fA/√Hz
  - - 100 nA/V
    - 60 fA/√Hz
  - - 1 μA/V
    - 300 fA/√Hz
  - - 10 μA/V
    - 3 pA/√Hz
  - - 100 μA/V
    - 30 pA/√Hz
  - - 1 mA/V
    - 300 pA/√Hz
  - - 10 mA/V
    - 3 nA/√Hz
  - - 50 mA/V
    - 15 nA/√Hz
---

The Model CS580 Voltage Controlled Current Source creates new capabilities for researchers needing an ultra-low noise current source in a flexible, easy to use instrument. The CS580 is a natural companion with sensitive AC instruments such as lock-in amplifiers, providing an easy way of generating precision current directly from an AC or DC control voltage. The instrument sources and sinks current and has an adjustable compliance voltage of up to ±50 V, giving full "four-quadrant" performance . The CS580 is a welcome addition to any research lab studying semiconductors and transport phenomena, superconductivity, and nanotechnology, to name just a few.
